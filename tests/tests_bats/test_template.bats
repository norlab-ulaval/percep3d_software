...
@test "p3sc::this_is_not_cool (explicitly source $TESTED_FILE) › expect fail" {
  run bash -c "source ./$TESTED_FILE && p3sc::this_is_not_cool"
  assert_failure 1
  assert_output --partial 'Noooooooooooooo!'
}

@test "p3sc::this_is_not_cool (source at setup step) › expect fail" {
  run p3sc::this_is_not_cool
  assert_failure 1
  assert_output --partial 'Noooooooooooooo!'
}

@test "p3sc::good_morning_norlab (environment variable not set) › expect fail" {
  run "p3sc::good_morning_norlab"
  assert_failure 1
  assert_output --partial 'Error: Environment variable not set'
  unset GREETING
}

@test "p3sc::good_morning_norlab (environment variable set) › expect pass" {
...
@test "p3sc::good_morning_norlab (command executed in a subshell) › expect pass" {
  assert_empty $GREETING
  run bash -c "source ./$TESTED_FILE && GREETING='Goooooooood morning NorLab' && p3sc::good_morning_norlab"
  assert_empty $GREETING
  assert_success
  assert_output --partial "Goooooooood morning NorLab ... there's nothing like the smell of a snow storm in the morning!"
}

@test "p3sc::talk_to_me_or_not › expect fail" {
  # Note:
...
#  run bash -c "yes 1 | p3sc::talk_to_me_or_not"
  run bash -c "source ./$TESTED_FILE && yes 1 | p3sc::talk_to_me_or_not"
  assert_failure 1
